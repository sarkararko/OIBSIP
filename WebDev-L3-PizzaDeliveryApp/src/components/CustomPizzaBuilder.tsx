...
export const CustomPizzaBuilder: React.FC<CustomPizzaBuilderProps> = ({
  ...
  const handleAddToCart = () => {
    if (!selectedBase) {
      alert('Please select a Pizza Base in Step 1.');
      setCurrentStep(1);
      return;
    }
    if (!selectedSauce) {
      alert('Please select a Sauce in Step 2.');
      setCurrentStep(2);
      return;
    }
    if (!selectedCheese) {
      alert('Please select a Cheese in Step 3.');
      setCurrentStep(3);
      return;
    }
  ...
  const getSauceColor = () => {
    ...
    if (selectedSauce.id.includes('pesto')) return '#16a34a';
    if (selectedSauce.id.includes('bbq') || selectedSauce.id.includes('chipotle')) return '#7f1d1d';
    if (selectedSauce.id.includes('garlic_butter') || selectedSauce.id.includes('alfredo')) return '#fef08a';
    if (selectedSauce.id.includes('arrabbiata')) return '#b91c1c';
    return '#dc2626'; // Classic Tomato
  };

  const getCheeseColor = () => {
    if (!selectedCheese) return '#fffbeb';
    if (selectedCheese.id.includes('cheddar')) return '#fde047';
    if (selectedCheese.id.includes('parmesan')) return '#fef3c7';
    if (selectedCheese.id.includes('ricotta')) return '#ffffff';
    if (selectedCheese.id.includes('vegan')) return '#f5f5f4';
    return '#fffbeb'; // Mozzarella
  };
              ...
              <div 
                className={`relative rounded-full transition-all duration-500 shadow-2xl flex items-center justify-center border-8 ${
                  selectedBase?.id === 'base_cheese_burst' 
                    ...
                    : selectedBase?.id === 'base_whole_wheat'
                    ? 'border-stone-700 bg-amber-800/30'
                    : selectedBase?.id === 'base_stuffed_crust'
                    ? 'border-orange-500 bg-orange-100 shadow-orange-500/20'
